...
export default function Flashcards() {
  ...
  const [cardForm, setCardForm] = useState({ front: "", back: "", conceptName: "" });
  ...
  const handleCreateCard = async (e) => {
    ...
    if (!cardForm.front.trim() || !cardForm.back.trim()) {
      toast.push("Please provide both front prompt and back answer", "error");
      return;
    }
    try {
      const res = await api.post(`/api/projects/${id}/flashcards`, cardForm);
      setDeck((prev) => [res.data.data, ...prev]);
      setCurrentIndex(0);
      setFlipped(false);
      setIsCreateOpen(false);
      setCardForm({ front: "", back: "", conceptName: "" });
      toast.push("Custom card added to deck!", "success");
    } catch (err) {
      toast.push(err.response?.data?.message || "Failed to create card", "error");
    }
  };
  ...
  const handleEditCard = async (e) => {
    ...
    if (!currentCard) return;
    try {
      const res = await api.put(`/api/projects/${id}/flashcards/${currentCard.id}`, cardForm);
      const updated = res.data.data;
      setDeck((prev) => prev.map((c) => (c.id === updated.id ? updated : c)));
      setIsEditOpen(false);
      toast.push("Card updated successfully", "success");
    } catch (err) {
      toast.push(err.response?.data?.message || "Failed to update card", "error");
    }
  };
  ...
  const handleDeleteCard = async (cardId) => {
    ...
    try {
      await api.delete(`/api/projects/${id}/flashcards/${cardId}`);
      setDeck((prev) => prev.filter((c) => c.id !== cardId));
      if (currentIndex >= deck.length - 1) {
        setCurrentIndex(Math.max(0, deck.length - 2));
      }
      setFlipped(false);
      toast.push("Card removed from deck", "info");
    } catch (err) {
      toast.push(err.response?.data?.message || "Failed to delete card", "error");
    }
  };
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 shadow-card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="inline-flex items-center space-x-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 text-xs font-semibold px-3 py-2 rounded-xl shadow-xs transition"
          >
          ...
          >
            ...
            <span>{generating ? "Synthesizing…" : "AI Generate"}</span>
          </button>
        </div>
      </div>
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center space-x-2 text-slate-400 text-[11px]">
                    ...
                    <button
                      ...
                      title="Edit card"
                      className="p-1 hover:text-slate-700 hover:bg-slate-100 rounded transition ml-1"
                    >
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-lg w-full p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="font-bold text-base text-slate-900">
                {isCreateOpen ? "Create Custom Flashcard" : "Edit Flashcard"}
              </h3>
              <button
                type="button"
                onClick={() => {
            ...
            <form onSubmit={isCreateOpen ? handleCreateCard : handleEditCard} className="space-y-3.5">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Concept Tag</label>
                <input
                  type="text"
                  value={cardForm.conceptName}
                  onChange={(e) => setCardForm({ ...cardForm, conceptName: e.target.value })}
                  placeholder="e.g. Graph Theory, Neural Networks, Thermodynamics"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Front (Question / Prompt) *</label>
                <textarea
                  rows={3}
                  required
                  value={cardForm.front}
                  onChange={(e) => setCardForm({ ...cardForm, front: e.target.value })}
                  placeholder="What is the time complexity of Dijkstra's algorithm?"
                  className="w-full text-xs px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-hidden focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Back (Answer / Derivation) *</label>
                <textarea
                  rows={4}
                  required
                  value={cardForm.back}
                  onChange={(e) => setCardForm({ ...cardForm, back: e.target.value })}
                  placeholder="O((V + E) log V) with a binary min-heap..."
                  className="w-full text-xs px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-hidden focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500"
                />
              </div>
              ...
              <div className="flex items-center justify-end space-x-2 pt-2">
                ...
                >
                  {isCreateOpen ? "Create Card" : "Save Changes"}
                </button>
              </div>
            </form>
          </div>
